...
public:
	...
	void load(std::string filename) {
		...
		texels = new Colour[width * height];
		for (int i = 0; i < (width * height); i++) {
			texels[i] = Colour(textureData[i * channels] / 255.f, textureData[(i * channels) + 1] / 255.f, textureData[(i * channels) + 2] / 255.f);
		}
		if (channels == 4) {
			alpha = new float[width * height];
			for (int i = 0; i < (width * height); i++) {
				alpha[i] = textureData[(i * channels) + 3] / 255.f;
			}
		}
		stbi_image_free(textureData);
	}
...
private:
	...
	float separable(float d) const {
		if (fabs(d) >= 0.f && fabs(d) < 1.f) {
			...
			return (1.f / 6.f) * (-B - 6.f * C) * powf(fabs(d), 3) +
				   (6.f * B + 30.f * C) * powf(fabs(d), 2) +
				   (-12.f * B -48.f * C) * fabs(d) +
				   (8.f * B + 24.f * C);
		}
		else return 0.f;
	}
